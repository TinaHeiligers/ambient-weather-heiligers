...
function convertToJsonl() {
  const allConvertedFiles = getArrayOfFiles('ambient-weather-heiligers-data-jsonl');
  ...
  if (filesToConvert.length > 0) {
    filesToConvert.forEach((entry, index) => {
      ...
      const openedDataForDailyFile = fs.openSync(`./data/ambient-weather-heiligers-data-jsonl/${entry}.jsonl`, "w");
  ...
  if (convertedCount === filesToConvert.length) {
    ...
    console.log(`couldn't convert files for ${filesToConvert.length - convertedCount} files`)
  }
}
